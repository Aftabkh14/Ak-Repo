odoo.define('account_partner_ledger_report.account_report_generic', function (require) {
'use strict';
...
var _t = core._t;
    ...
    var M2MAccountFilters = Widget.extend(StandaloneFieldManagerMixin, {
    /**
     * @constructor
     * @param {Object} fields
     */
    ...
    _makeM2MWidget: function (fieldInfo, fieldName) {
        ...
        var domain = [];
        if (this.__parentedParent.report_model =='account.partner.ledger'){
            domain = [['user_type_id.type', 'in' ,['receivable', 'payable']]]
//           this.__parentedParent.report_options.account_type.forEach(function (acc_type) {
//            console.log('fffffffff',acc_type)
//            if(acc_type.selected==true){
//                 domain = [['user_type_id.type', 'in' ,[acc_type.id]]]
//            }

//        });

        }
        else if(this.__parentedParent.report_model =='account.aged.receivable'){
            domain = [['user_type_id.type', 'in' ,['receivable']]]
        }
        else if(this.__parentedParent.report_model =='account.aged.payable'){
            domain = [['user_type_id.type', 'in' ,['payable']]]
        }
        var returned_values = {}

        if (fieldInfo.modelName == 'account.account'){
           returned_values = {
        ...
        return this.model.makeRecord(fieldInfo.modelName, [returned_values], options).then(function (recordID) {
    ...
    accountReportsWidget.include({
         custom_events: _.extend({}, accountReportsWidget.prototype.custom_events, {
            ...
            'value_changed': function(ev) {
                var self = this;
                self.report_options.account_ids = ev.data.account;
...
//
//                    if (!_.isEmpty(fields)) {
//                        this.M2MAccountFilters = new M2MAccountFilters(this, fields);
//                        this.M2MAccountFilters.appendTo(this.$searchview_buttons.find('.js_account_account_m2m'));
//                    }
//                } else {
//                    this.$searchview_buttons.find('.js_account_account_m2m').append(this.M2MAccountFilters.$el);
//                }
//            }
//
//
...
//        },
        ...
        if (this.report_options.partner) {
            if (!this.M2MAccountFilters) {
                ...
                if (!_.isEmpty(fields)) {
                    this.M2MAccountFilters = new M2MAccountFilters(this, fields);
                    this.M2MAccountFilters.appendTo(this.$searchview_buttons.find('.js_account_partner_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_partner_m2m').append(this.M2MAccountFilters.$el);
            }
        }
...
//                if (this.report_options.branch_ids) {
//                    fields['branch'] = {
//                        label: _t('Branch'),
...
//                }
                ...
                if (!_.isEmpty(fields)) {
                    this.M2MAccountFilters = new M2MAccountFilters(this, fields);
                    this.M2MAccountFilters.appendTo(this.$searchview_buttons.find('.js_account_analytic_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_analytic_m2m').append(this.M2MAccountFilters.$el);
            }
        }
    },
    });
